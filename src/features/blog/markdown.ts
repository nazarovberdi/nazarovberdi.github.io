...
async function renderCodeFence(code: string, language: string): Promise<string> {
...
function formatInline(markdown: string): string {
  ...
  return codeSpans.reduce(
    (result, replacement, index) => result.replace(`__CODE_SPAN_${index}__`, replacement),
    html,
  )
}
...
export function parseFrontmatter(raw: string): ParsedMarkdownFile {
  ...
  for (const line of lines.slice(1, closingIndex)) {
    ...
    const key = line.slice(0, separatorIndex).trim() as keyof FrontmatterAttributes
...
export async function markdownToHtml(markdown: string): Promise<string> {
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.startsWith('```')) {
      ...
      if (inCodeFence) {
        output.push(await renderCodeFence(codeFenceLines.join('\n'), codeFenceLanguage))
        inCodeFence = false
        codeFenceLanguage = ''
        codeFenceLines = []
      } else {
    ...
    if (blockquoteMatch) {
      flushAll()
      output.push(`<blockquote><p>${formatInline(blockquoteMatch[1] ?? '')}</p></blockquote>`)
      continue
    }
